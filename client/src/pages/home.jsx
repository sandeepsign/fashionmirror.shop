...
function Dashboard() {
    ...
    const [selectedFashionItems, setSelectedFashionItems] = useState([]);
    const [allFashionItems, setAllFashionItems] = useState([]);
    const [itemSelectionStates, setItemSelectionStates] = useState([]);
    ...
    const handleModelImageSelect = (file) => {
        setModelImage(file);
        if (file)
            setCurrentStep(2);
    };
    const handleFashionImagesSelect = (files) => {
        setFashionImages(files);
        // Convert uploaded files to FashionItemInput format
        const newFashionItems = files.map((file, index) => ({
            image: file,
    ...
    const handleFashionItemSelect = (item) => {
        ...
        const collectionFashionItems = newSelectedItems.map(collectionItem => ({
    ...
    const handleGenerationProgress = (completed, total) => {
    ...
    const handleItemSelectionToggle = (index) => {
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <section className="gradient-bg py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-4xl mx-auto">
            ...
            {verificationMessage && (<div className="mb-8">
                <Alert className={verificationMessage.type === 'success' ? 'border-green-200 bg-green-50' :
                verificationMessage.type === 'error' ? 'border-red-200 bg-red-50' :
                    'border-blue-200 bg-blue-50'}>
                  <AlertDescription className={verificationMessage.type === 'success' ? 'text-green-700' :
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button className="bg-primary text-primary-foreground px-8 py-4 text-lg font-medium hover:opacity-90" onClick={scrollToUpload} data-testid="button-start-try-on">
                Start Virtual Try-On
              </Button>
              <Button variant="outline" className="px-8 py-4 text-lg font-medium" onClick={() => {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div id="upload-section" className="grid lg:grid-cols-2 gap-12 mb-16">
          <ModelUpload onImageSelect={handleModelImageSelect} selectedImage={modelImage}/>
          <FashionUpload onImagesSelect={handleFashionImagesSelect} selectedImages={fashionImages} onBrowseCollection={handleBrowseCollection}/>
        </div>

        {/* Fashion Collection */}
        <div id="fashion-collection">
          <FashionCollection onItemSelect={handleFashionItemSelect} selectedItems={selectedFashionItems}/>
        </div>

        {/* Try-On Workspace */}
        <div id="try-on-workspace">
          <TryOnWorkspace modelImage={modelImage} allFashionItems={allFashionItems} selectedFashionItems={selectedFashionItems} itemSelectionStates={itemSelectionStates} selectedItemsCount={getSelectedItemsCount()} onResultsGenerated={handleResultsGenerated} onGenerationProgress={handleGenerationProgress} onGenerationStart={handleGenerationStart} onGenerationEnd={handleGenerationEnd} onItemRemove={handleItemRemove} onItemSelectionToggle={handleItemSelectionToggle} getSelectedFashionItems={getSelectedFashionItems} isGenerating={isGenerating} generationProgress={generationProgress}/>
        </div>
