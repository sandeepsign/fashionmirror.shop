...
export default function MerchantSettings() {
    ...
    return (<DashboardLayout title="Settings">
      {/* Notifications */}
      {successMessage && (<Alert className="mb-6 bg-green-900/50 border-green-800 text-green-200">
          <AlertDescription>{successMessage}</AlertDescription>
        </Alert>)}
      {errorMessage && (<Alert variant="destructive" className="mb-6 bg-red-900/50 border-red-800 text-red-200">
        ...
        <Card className="bg-slate-800/50 border-slate-700">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your business name" className="bg-slate-700/50 border-slate-600 text-white placeholder:text-slate-500"/>
            </div>

            <div className="space-y-2">
              <Label htmlFor="email" className="text-slate-200">Email</Label>
              <Input id="email" value={merchant?.email || ''} disabled className="bg-slate-900/50 border-slate-700 text-slate-400"/>
        ...
        <Card className="bg-slate-800/50 border-slate-700">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="webhookUrl" type="url" value={webhookUrl} onChange={(e) => setWebhookUrl(e.target.value)} placeholder="https://your-domain.com/webhook" className="bg-slate-700/50 border-slate-600 text-white placeholder:text-slate-500"/>
            ...
            <div className="space-y-2">
              ...
              {webhookSecret ? (<div className="flex items-center gap-2">
                  ...
                  <Button variant="outline" size="icon" onClick={copyWebhookSecret} className="border-slate-600 hover:bg-slate-700">
                    {copiedSecret ? (<Check className="h-4 w-4 text-green-400"/>) : (<Copy className="h-4 w-4 text-slate-400"/>)}
                  </Button>
              ...
              <div className="flex items-center justify-between">
                ...
                <Button variant="outline" size="sm" onClick={handleRegenerateWebhookSecret} disabled={isRegeneratingSecret} className="border-slate-600 text-slate-300 hover:bg-slate-700">
