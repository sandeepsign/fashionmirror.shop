...
export default function MerchantAnalytics() {
    ...
    const getStatusBadge = (status) => {
        switch (status) {
            case 'completed':
                return (<span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-green-900/50 text-green-400">
            <CheckCircle className="w-3 h-3"/>
            Completed
          </span>);
            case 'failed':
                return (<span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-red-900/50 text-red-400">
            <XCircle className="w-3 h-3"/>
            Failed
          </span>);
            case 'processing':
                return (<span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-blue-900/50 text-blue-400">
            <Loader2 className="w-3 h-3 animate-spin"/>
            Processing
          </span>);
            default:
                return (<span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-slate-700 text-slate-400">
    ...
    return (<DashboardLayout title="Analytics">
      {isLoading ? (<div className="flex items-center justify-center py-12">
          ...
          <Card className="bg-slate-800/50 border-slate-700 mb-8">
            ...
            <CardContent>
              <div className="h-48 flex items-end gap-2">
                {chartData.map((day) => (<div key={day.date} className="flex-1 flex flex-col items-center gap-1">
                    <div className="w-full bg-indigo-600 rounded-t transition-all hover:bg-indigo-500" style={{
          ...
          <Card className="bg-slate-800/50 border-slate-700">
            ...
            <CardContent>
              {sessions.length === 0 ? (<div className="text-center py-12 text-slate-400">
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {sessions.map((session) => (<tr key={session.id} className="border-b border-slate-700/50 hover:bg-slate-700/20">
                          <td className="py-3 px-4">
                            <div className="flex items-center gap-3">
                              {session.productImage ? (<img src={session.productImage} alt={session.productName || 'Product'} className="w-10 h-10 rounded object-cover bg-slate-700"/>) : (<div className="w-10 h-10 rounded bg-slate-700 flex items-center justify-center">
                              ...
                              <div>
                                ...
                                {session.productId && (<p className="text-xs text-slate-500">{session.productId}</p>)}
                              </div>
                            </div>
                          </td>
                          <td className="py-3 px-4">
