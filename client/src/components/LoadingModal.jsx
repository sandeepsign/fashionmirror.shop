export default function LoadingModal({ isOpen, progress = 0 }) {
    if (!isOpen)
        return null;
    return (<div className="fixed inset-0 loading-overlay flex items-center justify-center z-50" data-testid="loading-modal">
      <div className="bg-card rounded-2xl p-8 text-center space-y-6 border border-border shadow-2xl max-w-md mx-4">
        ...
        <div className="space-y-2">
          <div className="w-full h-2 bg-border rounded-full">
            <div className="h-2 bg-primary rounded-full transition-all duration-500" style={{ width: `${progress}%` }} data-testid="progress-bar"/>
          </div>
