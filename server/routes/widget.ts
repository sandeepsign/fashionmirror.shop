...
import {
  ...
  triggerTryOnProcessing,
  triggerTryOnCompleted,
  triggerTryOnFailed,
 ...
 */
router.post("/verify", widgetAuth, async (req: Request, res: Response) => {
  try {
 ...
 */
router.post("/session", widgetAuth, async (req: Request, res: Response) => {
  try {
    const user = req.widgetUser!;
 ...
 */
router.get("/session/:id", widgetAuth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const user = req.widgetUser!;
 ...
 */
router.delete("/session/:id", widgetAuth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const user = req.widgetUser!;
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (photoFile) {
        userPhotoBuffer = photoFile.buffer;
        ...
        try {
          ...
          userPhotoBuffer = Buffer.from(buffer);
          userPhotoBase64 = userPhotoBuffer.toString("base64");
        } catch (fetchError) {
          await storage.updateWidgetSession(sessionId, {
            status: "failed",
      ...
      // Save user photo to filesystem and update session
      let savedUserImageUrl: string | null = null;
      if (userPhotoBuffer) {
        try {
          const savedImage = await mediaStorage.saveImage(userPhotoBuffer, {
            visibility: "protected",
            type: "model",
            userId: user.id,
            ext: "jpg",
          });
          savedUserImageUrl = savedImage.url;

          // Update session with user image URL
          await storage.updateWidgetSession(sessionId, {
            userImage: savedUserImageUrl,
          });
        } catch (saveError) {
          console.error("Error saving user photo:", saveError);
 ...
 * Track widget events
 */
router.post("/analytics", widgetAuth, async (req: Request, res: Response) => {
  try {
    const user = req.widgetUser!;
