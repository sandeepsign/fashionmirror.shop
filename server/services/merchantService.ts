...
import type { User, UserPublic, planLimits, PlanType } from "@shared/schema";
 ...
 */
export function generateUserApiKeys(): { liveKey: string; testKey: string } {
  return {
 ...
 */
export function isQuotaExceeded(user: User): boolean {
 ...
 */
export function shouldResetQuota(user: User): boolean {
 ...
 */
export function getEffectiveQuota(user: User): { used: number; limit: number; isLifetime: boolean } {
  const used = user.quotaUsed ?? 0;
