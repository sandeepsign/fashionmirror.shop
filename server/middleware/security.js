 ...
 */
export function rateLimitByMerchant(req, res, next) {
    const merchantKey = req.headers['x-merchant-key'] || 'unknown';
...
export async function widgetCors(req, res, next) {
    ...
    if (merchantKey.startsWith('mk_test_')) {
        ...
        if (origin) {
            const isDevOrigin = allowedDevOrigins.some((dev) => origin.includes(dev));
            if (isDevOrigin) {
                res.setHeader('Access-Control-Allow-Origin', origin);
    ...
    // For live keys, validate against merchant's allowed domains
    if (merchantKey.startsWith('mk_live_') && origin) {
        try {
            const storage = await getStorage();
            const merchant = await storage.getMerchantByLiveKey(merchantKey);
            if (merchant && merchant.allowedDomains) {
                const originUrl = new URL(origin);
                const originDomain = originUrl.hostname;
                const allowedDomains = merchant.allowedDomains;
 ...
 */
export function iframeCsp(req, res, next) {
...
export function securityHeaders(req, res, next) {
    ...
    res.setHeader('Permissions-Policy', 'accelerometer=(), ambient-light-sensor=(), autoplay=(), battery=(), camera=(self), display-capture=(), document-domain=(), encrypted-media=(), fullscreen=(), geolocation=(), gyroscope=(), magnetometer=(), microphone=(self), midi=(), payment=(), picture-in-picture=(), publickey-credentials-get=(), screen-wake-lock=(), sync-xhr=(), usb=(), xr-spatial-tracking=()');
    next();
}
/**
 * Performance headers for static assets
 */
export function staticCacheHeaders(maxAge = 86400) {
    return (req, res, next) => {
        // Cache static assets
 ...
 */
export function widgetAssetHeaders(req, res, next) {
