...
import { isValidApiKeyFormat, isTestKey, isDomainAllowed, extractDomain, isQuotaExceeded, shouldResetQuota, getNextQuotaResetDate, } from "../services/merchantService";
 ...
 */
export async function widgetAuth(req, res, next) {
    try {
        ...
        const userRateLimit = checkRateLimit(`user:${user.id}`, RATE_LIMITS.perMerchant.requests, RATE_LIMITS.perMerchant.windowMs);
        ...
        const ipRateLimit = checkRateLimit(`ip:${clientIp}`, RATE_LIMITS.perIp.requests, RATE_LIMITS.perIp.windowMs);
 ...
 */
export async function optionalWidgetAuth(req, res, next) {
    const apiKey = req.headers["x-merchant-key"];
    if (!apiKey) {
        next();
        return;
