...
import { AppError, ErrorCodes, logError, isOperationalError } from '../utils/errors';
...
export function requestLogger(req: Request, res: Response, next: NextFunction) {
  ...
  res.on('finish', () => {
    ...
    console[logLevel](
      `[${new Date().toISOString()}] ${req.method} ${req.path} - ${res.statusCode} - ${duration}ms - Request ID: ${req.requestId}`
    );
  });
